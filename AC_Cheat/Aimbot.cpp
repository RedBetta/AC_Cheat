...
void Aimbot::MoveCrosshairTo(Vector3 otherPlayerPos)
{
...
int Aimbot::FindVisibleEnemyPlayers(std::vector<Player*>& players, std::vector<Player*>& visiblePlayers, int nbOfPlayers)
{
    ...
    for (int i = 1; i < nbOfPlayers; i++)
    {
        ...
        if (players[i]->isVisible(players[0]) && players[i]->state == 0 && (AC::IsTeamGame() && AC::IsEnemy(players[i]) || !AC::IsTeamGame()))
        {
...
Player* Aimbot::FindClosestPlayer(std::vector<Player*>& players, int nbOfPlayers)
{
